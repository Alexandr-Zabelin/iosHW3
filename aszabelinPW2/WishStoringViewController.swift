...
extension WishStoringViewController: UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        wishCell.configure(with: wishes[indexPath.row], index: indexPath.row, action: {[weak self] idxToRemove in
            if (idxToRemove == nil || self?.wishes == nil) {
                return
            }
            
            var newWishes: [String] = []
            
            for idx in 0 ..< (self?.wishes.count ?? 0) {
                if (idx != idxToRemove) {
                    newWishes.append(self!.wishes[idx])
                }
            }
